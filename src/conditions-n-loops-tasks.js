...
function canQueenCaptureKing(queen, king) {
  if (
    queen.x === king.x ||
    queen.y === king.y ||
    Math.abs(queen.x - king.x) === Math.abs(king.y - queen.y)
  ) {
    return true;
  }
  return false;
 ...
 *  26  => XXVI
 */
function convertToRomanNumerals(data) {
  let num = data;
  const romanNums = [
...
function rotateMatrix(data) {
  ...
  for (let i = 0; i < matrix.length; i += 1) {
    for (let j = 0; j < Math.floor(matrix.length / 2); j += 1) {
      ...
      matrix[i][matrix.length - 1 - j] = val;
    }
  }
  return matrix;
}
